...
export default async function MemberExplorePage() {
  ...
  const classInstructors = new Map(
    classTypes.map(([classType]) => [
      classType,
      Array.from(new Set(schedule.filter((session) => session.class_type === classType).map((session) => session.instructor_name))).sort(),
    ]),
  );
  ...
  return (
    <PortalShell audience="member" eyebrow="Member portal" title="Explore" description="Discover classes and instructors." links={memberLinks} showHeader={false}>
      ...
      {error ? (
        <div role="alert" className="mt-4 rounded-3xl border border-[#c72c25]/25 bg-white/70 p-5 text-sm text-[#8e211c]">Class and instructor information is temporarily unavailable.</div>
      ) : schedule.length === 0 ? (
        ...
        <>
          <section className="mt-4 rounded-3xl border border-white/70 bg-white/68 p-5 shadow-[0_1rem_3rem_rgba(17,17,17,0.06)] backdrop-blur-xl sm:p-6" aria-labelledby="explore-classes-title">
            ...
            <h2 id="explore-classes-title" className="mt-2 text-2xl font-semibold tracking-[-0.04em]">Explore by class and instructor</h2>
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {classTypes.map(([value, label]) => (
                <article key={value} className="flex h-full flex-col rounded-3xl border border-black/10 bg-[#f7f4ee] p-5">
                  ...
                  <p className="mt-1 text-sm text-black/65">Select an instructor to open only their {label} sessions.</p>
                  <div className="mt-4 grid gap-2">
                    {classInstructors.get(value)?.map((instructor) => (
                      <Link key={instructor} href={`/member/classes?class=${value}&instructor=${encodeURIComponent(instructor)}`} className="inline-flex min-h-11 items-center justify-between rounded-full border border-black/15 bg-white/75 px-4 text-sm font-semibold transition hover:border-black hover:bg-black hover:text-white focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2">
                        <span>{instructor}</span><ArrowUpRight className="size-4" aria-hidden="true" />
                      </Link>
                    ))}
                  </div>
                  <Link href={`/member/classes?class=${value}`} className="mt-auto inline-flex min-h-11 items-end pt-3 text-sm font-semibold underline decoration-[#c72c25] decoration-2 underline-offset-4 focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2">View all {label} classes</Link>
                </article>
              ))}
            </div>
          </section>
