  ...
  return <PortalShell audience="member" eyebrow="Member portal" title="Account" description="Membership and account information." links={memberLinks} showHeader={false}>
    {unavailable ? <div role="alert" className="rounded-3xl border border-black/15 bg-white/65 p-6 text-[#8e211c]">Your account information is temporarily unavailable.</div> : <>
      <header className="rounded-3xl bg-[#171717] p-6 text-white shadow-[0_1.5rem_4rem_rgba(17,17,17,0.18)]">
        <div className="flex items-start gap-4"><Image src="/media/portraits/member-maya.png" alt={`${member.first_name} ${member.last_name}`} width={96} height={96} sizes="48px" className="size-12 shrink-0 rounded-full border border-white/15 object-cover" /><div><p className="route-eyebrow text-white/65">Account information</p><h1 className="route-title mt-2 text-3xl sm:text-4xl">{member.first_name} {member.last_name}</h1><p className="mt-2 text-sm capitalize text-white/70">{summary.membership_status} member · ID {memberId}</p></div></div>
      </header>
