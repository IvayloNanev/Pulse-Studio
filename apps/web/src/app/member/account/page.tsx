...
export default async function MemberAccountPage() {
  ...
  return <PortalShell audience="member" eyebrow="Member portal" title="Account" description="Membership and account information." links={memberLinks} showHeader={false}>
    {unavailable ? <div role="alert" className="rounded-3xl border border-[#c72c25]/30 bg-white/65 p-6 text-[#8e211c]">Your account information is temporarily unavailable.</div> : <>
      <header className="rounded-3xl bg-[#171717] p-6 text-white shadow-[0_1.5rem_4rem_rgba(17,17,17,0.18)]">
        <div className="flex items-start gap-4"><div className="inline-flex size-12 shrink-0 items-center justify-center rounded-full border border-white/15 bg-white/8"><UserRound className="size-6" aria-hidden="true" /></div><div><p className="font-mono text-xs uppercase tracking-[0.16em] text-white/65">Account information</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.05em] sm:text-4xl">{member.first_name} {member.last_name}</h1><p className="mt-2 text-sm capitalize text-white/70">{summary.membership_status} member · ID {memberId}</p></div></div>
      </header>

      <section className="mt-4 grid gap-4 lg:grid-cols-2" aria-label="Account details">
        ...
        <article className="rounded-3xl border border-white/70 bg-white/68 p-5 shadow-[0_1rem_3rem_rgba(17,17,17,0.06)] backdrop-blur-xl sm:p-6"><p className="font-mono text-xs uppercase tracking-[0.14em] text-black/60">Contact information</p><dl className="mt-4 space-y-4 text-sm"><div><dt className="text-black/60">Email</dt><dd className="mt-1 break-all font-semibold"><a href={`mailto:${member.email}`} className="underline decoration-[#c72c25] decoration-2 underline-offset-4">{member.email}</a></dd></div><div><dt className="text-black/60">Phone</dt><dd className="mt-1 font-semibold">{member.phone ? <a href={`tel:${member.phone}`} className="underline decoration-[#c72c25] decoration-2 underline-offset-4">{member.phone}</a> : "Not provided"}</dd></div><div><dt className="text-black/60">Preferred contact</dt><dd className="mt-1 capitalize font-semibold">{member.preferred_channel}</dd></div></dl></article>
