...
import { StaffAttentionQueue } from "@/components/staff-attention-queue";
...
import { sortAttendanceAttention, type AttendanceAction } from "@/lib/product-b/attention-priority";
...
type AttendanceEligibility = { class_session_id: string; reservation_status: "confirmed" | "waitlisted"; attendance_status: "attended" | "no_show" | null; can_record_attended: boolean; can_record_no_show: boolean };
...
export default async function StaffRostersPage({ searchParams }: { searchParams: Promise<{ instructor?: string; date?: string; roster?: string; success?: string; error?: string }> }) {
  ...
  const attentionThrough = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
  const [currentStaffResult, instructorsResult, sessionsResult, attentionSessionsResult, eligibilityResult] = await Promise.all([
    ...
    supabase.from("staff_product_b_sessions").select("class_session_id,class_type_label,starts_at,instructor_name,confirmed_reservations,waitlisted_reservations,capacity,marked_count,is_cancelled").gte("starts_at", attentionSince.toISOString()).lt("starts_at", attentionThrough.toISOString()).order("starts_at", { ascending: true }),
    supabase.from("staff_session_roster").select("class_session_id,reservation_status,attendance_status,can_record_attended,can_record_no_show").gte("starts_at", attentionSince.toISOString()).lt("starts_at", attentionThrough.toISOString()),
  ]);
  ...
  const attentionCandidates = (Array.isArray(attentionSessionsResult.data) ? attentionSessionsResult.data as Session[] : []).filter((session) => !session.is_cancelled && !session.instructor_name.startsWith("EOD ") && session.instructor_name !== "Reymundo Bermejo");
  const scopedAttentionCandidates = viewingAll ? attentionCandidates : attentionCandidates.filter((session) => session.instructor_name === instructorName);
  const actionBySession = new Map<string, AttendanceAction>();
  for (const item of (eligibilityResult.data ?? []) as AttendanceEligibility[]) {
    if (item.reservation_status !== "confirmed" || item.attendance_status) continue;
    if (item.can_record_no_show) actionBySession.set(item.class_session_id, "no_show");
    else if (item.can_record_attended && !actionBySession.has(item.class_session_id)) actionBySession.set(item.class_session_id, "attended");
  }
  const attentionSessions = sortAttendanceAttention(scopedAttentionCandidates.filter((session) => actionBySession.has(session.class_session_id)), actionBySession).map((session) => ({
    class_session_id: session.class_session_id,
    class_type_label: session.class_type_label,
    starts_at: session.starts_at,
    instructor_name: session.instructor_name,
    confirmed_reservations: session.confirmed_reservations,
    attendance_action: actionBySession.get(session.class_session_id)!,
  }));
