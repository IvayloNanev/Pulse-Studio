        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,0.8fr)_minmax(24rem,1.2fr)]">
          <div className="space-y-6">
            <section className="glass-panel rounded-3xl p-6">
              ...
              {detail.review_status === "pending" && <form action={startRiskReview} className="mt-5"><input type="hidden" name="risk_assessment_id" value={riskId} /><button className="min-h-11 rounded-full bg-black px-5 text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2">Start review</button></form>}
            </section>
            ...
            <section className="glass-panel rounded-3xl p-6">
              ...
              <form action={createRiskNote} className="mt-4"><input type="hidden" name="risk_assessment_id" value={riskId} /><label className="text-sm font-semibold" htmlFor="note-body">Add factual context</label><textarea id="note-body" name="body" required rows={3} className="mt-2 w-full rounded-xl border border-black/20 bg-white/60 p-3 focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2" /><button className="mt-3 min-h-11 rounded-full bg-black px-5 text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2">Add note</button></form>
              <div className="mt-5 space-y-3">{detail.active_notes.map((note) => <article key={note.note_id} className="border-t border-black/10 pt-3"><p className="text-sm leading-6">{note.body}</p><p className="mt-2 text-xs text-black/60">{note.author_name} · {formatter.format(new Date(note.created_at))}</p></article>)}</div>
            </section>
          </div>
