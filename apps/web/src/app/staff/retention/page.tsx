            ...
            <article key={item.risk_assessment_id} className="glass-panel grid gap-5 rounded-3xl p-6 md:grid-cols-[1fr_auto] md:items-center">
              <div>
                <div className="flex flex-wrap gap-2">
                  <span className={`rounded-full border bg-white/55 px-2.5 py-1 font-mono text-xs uppercase tracking-[0.12em] ${item.risk_level === "high" ? "border-black/20 text-[#a9231e]" : "border-amber-600 text-amber-800"}`}>{item.risk_level} risk</span>
                  <span className="rounded-full border border-black/20 bg-white/55 px-2.5 py-1 font-mono text-xs uppercase tracking-[0.12em]">{item.review_status.replace("_", " ")}</span>
                  <span className="rounded-full border border-black/20 bg-white/55 px-2.5 py-1 font-mono text-xs uppercase tracking-[0.12em]">Outreach: {item.outreach_status ?? "not started"}</span>
                </div>
                ...
                <p className="mt-1 text-sm text-black/65">Last attended: {item.last_attended_at ? formatter.format(new Date(item.last_attended_at)) : "No recorded attendance"} · {item.active_note_count} note{item.active_note_count === 1 ? "" : "s"}</p>
                {item.outreach_blocked_reason && <p className="mt-2 text-xs text-[#8e211c]">{item.outreach_blocked_reason}</p>}
              </div>
              <Link href={`/staff/retention/${encodeURIComponent(item.risk_assessment_id)}`} className="inline-flex min-h-11 items-center justify-center rounded-full bg-black px-5 text-sm font-semibold text-white transition hover:bg-[#c72c25] focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2">Review case</Link>
            </article>
          ))}
        </div>
      )}
