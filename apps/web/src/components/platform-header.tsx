...
function HeaderLinkItem({ href, label, pathname, mobile = false }: HeaderLink & { pathname: string; mobile?: boolean }) {
  ...
  return (
    <header className="platform-header">
      ...
      <nav className="hidden min-w-0 items-center gap-1 lg:flex xl:gap-4 2xl:gap-6" aria-label={navigationLabel}>
        {props.links.map((link) => <HeaderLinkItem key={`${link.href}-${link.label}`} {...link} pathname={pathname} />)}
        {isPublic ? (
      ...
      <details className="group relative lg:hidden">
        ...
        <nav aria-label={`${navigationLabel} mobile`} className="platform-mobile-menu">
          {mobileLinks.map((link) => <HeaderLinkItem key={`${link.href}-${link.label}`} {...link} pathname={pathname} mobile />)}
          {isPublic ? (
