...
export function Brand({ inverse = false, prominent = false }: { inverse?: boolean; prominent?: boolean }) {
  return (
    <Link
      href="/"
      className={`inline-flex items-center ${prominent ? "gap-5" : "gap-3"} ${inverse ? "text-white" : "text-black"}`}
      aria-label="Pulse Studio home"
    >
      <svg viewBox="0 0 64 40" className={prominent ? "h-16 w-24 overflow-visible" : "h-9 w-14 overflow-visible"} aria-hidden="true">
        <g fill="#c72c25">
      ...
      <span className="flex items-baseline gap-2">
        <span className={`pulse-wordmark uppercase leading-none ${prominent ? "text-5xl" : "text-[1.55rem]"}`}>Pulse</span>
