...
export function PulseAssistantChat() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => conversation.current?.scrollTo({ top: conversation.current.scrollHeight, behavior: "smooth" }));
  ...
  function submitQuestion(event: FormEvent<HTMLFormElement>) {
    ...
    const response = ranked[0]?.score > 0 ? ranked[0].policy.answer : "I don’t have an approved answer for that yet. Try asking about classes, preparation, booking, cancellations, or membership policies.";
    setMessages((current) => [...current, { id: nextId.current++, role: "member", text: question }, { id: nextId.current++, role: "assistant", text: response }]);
    setInput("");
  }

  return <>
    <button type="button" onClick={() => { if (!requested.current) setLoading(true); setOpen(true); }} aria-label="Open Pulse Assistant" className="fixed bottom-5 right-5 z-40 inline-flex min-h-12 items-center gap-2 rounded-full bg-[#c72c25] px-5 font-semibold text-white shadow-[0_1rem_2.5rem_rgba(99,20,17,0.32)] transition hover:bg-[#a9231e] focus-visible:outline-2 focus-visible:outline-white focus-visible:outline-offset-2"><MessageCircle className="size-5" aria-hidden="true" /><span className="hidden sm:inline">Pulse Assistant</span></button>
    {open ? <section role="dialog" aria-modal="false" aria-labelledby="pulse-assistant-title" className="fixed inset-3 z-50 flex h-[calc(100dvh-1.5rem)] flex-col overflow-hidden rounded-3xl border border-white/70 bg-[rgba(247,244,238,0.92)] shadow-[0_1.5rem_5rem_rgba(17,17,17,0.28),inset_0_1px_0_rgba(255,255,255,0.9)] backdrop-blur-2xl sm:inset-auto sm:bottom-5 sm:right-5 sm:h-[min(42rem,calc(100dvh-2.5rem))] sm:w-[25rem]">
      <header className="shrink-0 border-b border-black/10 bg-[#171717] px-5 py-4 text-white"><div className="flex items-center justify-between"><div><p className="font-mono text-[0.65rem] uppercase tracking-[0.16em] text-white/65">Member support</p><h2 id="pulse-assistant-title" className="mt-1 text-lg font-semibold">Pulse Assistant</h2></div><button type="button" onClick={() => setOpen(false)} aria-label="Close Pulse Assistant" className="inline-flex size-11 items-center justify-center rounded-full border border-white/20 focus-visible:outline-2 focus-visible:outline-white focus-visible:outline-offset-2"><X className="size-5" aria-hidden="true" /></button></div><p className="mt-1 flex items-center gap-2 text-xs text-white/65"><span className="size-2 rounded-full bg-emerald-400" aria-hidden="true" />Approved studio answers</p></header>
