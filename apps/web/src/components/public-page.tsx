...
export function PublicPage({ eyebrow, title, introduction, children, compact = false, heroImage, heroImageAlt = "" }: PublicPageProps) {
  return (
    <main className={`relative isolate min-h-screen text-[#111111] ${heroImage ? "bg-transparent" : "bg-[#f3f0e9]"}`}>
      ...
      <SiteHeader />
      <header className={`relative overflow-hidden px-6 sm:px-10 lg:px-16 ${heroImage ? `flex items-center text-white ${compact ? "min-h-[clamp(22rem,42svh,28rem)]" : "min-h-[calc(100svh-5rem)]"}` : "border-b border-black/15"} ${compact && !heroImage ? "py-10 lg:py-14" : compact ? "py-10 lg:py-12" : "py-16 lg:py-24"}`}>
