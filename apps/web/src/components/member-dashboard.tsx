    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <section className="rounded-3xl border border-black/10 bg-white/65 p-4 shadow-[0_1rem_3rem_rgba(17,17,17,0.06)] sm:p-6" aria-labelledby="schedule-title">
        ...
        <div className="mt-6">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-semibold text-black/65" aria-label="Class calendar legend">{Object.entries(classCalendarStyle).map(([type, style]) => <span key={type} className="inline-flex items-center gap-1.5"><span className={`size-2.5 rounded-full ${style.dot}`} aria-hidden="true" />{style.label}</span>)}<span className="inline-flex items-center gap-1.5"><span className="inline-flex size-3 items-center justify-center rounded-full bg-black text-[0.5rem] text-white" aria-hidden="true">✓</span>Reserved</span><span className="inline-flex items-center gap-1.5"><span className="size-2.5 rounded-full bg-black/35 ring-2 ring-amber-600 ring-offset-1" aria-hidden="true" />Amber ring = waitlist available</span></div>
