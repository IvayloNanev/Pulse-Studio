...
export function MemberDashboard({ calendarDays, dataFetchedAt, eligibilityError, initialClassType = "all", initialDay, initialInstructor = "all", reservationError, reservations, scheduleError, sessions, summary }: MemberDashboardProps) {
  const firstAvailableDay = calendarDays.find((date) => sessions.some((session) => dayKey(session.starts_at) === date.key));
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <header className="flex flex-col justify-between gap-3 rounded-3xl bg-[#171717] p-4 text-white shadow-[0_1.25rem_3rem_rgba(17,17,17,0.14)] sm:flex-row sm:items-center sm:p-5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">{summary ? <><span className="inline-flex items-center gap-2 capitalize"><span className={`size-2 rounded-full ${canBook ? "bg-emerald-400" : "bg-amber-400"}`} aria-hidden="true" />{summary.membership_status}</span><span><strong>{summary.classes_remaining}</strong> current-cycle credits available</span></> : <span>Booking eligibility unavailable</span>}</div>
      </header>
      ...
      <section className="rounded-3xl border border-black/10 bg-white/65 p-4 shadow-[0_1rem_3rem_rgba(17,17,17,0.06)] sm:p-6" aria-labelledby="schedule-title">
        <div className="flex flex-col justify-between gap-3 xl:flex-row xl:items-end">
          <div>
            <p className="flex items-center gap-2 font-mono text-xs uppercase tracking-[0.16em] text-black/65"><CalendarDays className="size-4 text-[#c72c25]" aria-hidden="true" /> Current schedule · next 7 days</p>
        ...
        <div className="flex snap-x scroll-px-1 gap-2 overflow-x-auto px-1 pb-2 pr-8" aria-label="Select a day">
          {calendarDays.map((date, index) => {
            ...
            return <button key={date.key} type="button" aria-pressed={selected} aria-label={`${index === 0 ? "Today, " : ""}${fullDayFormatter.format(parsedDate)}, ${sessionCount} ${sessionCount === 1 ? "class" : "classes"}`} onClick={() => setSelectedDay(date.key)} className={`min-h-24 min-w-[5.25rem] snap-start rounded-2xl border px-3 py-3 text-center transition focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2 ${selected ? "border-black bg-black text-white" : "border-black/15 bg-[#f7f4ee] text-black hover:border-black/45"}`}><span className="block text-xs font-semibold uppercase tracking-[0.1em] opacity-75">{index === 0 ? "Today" : weekdayFormatter.format(parsedDate)}</span><span className="mt-1 block text-xl font-semibold">{dayFormatter.format(parsedDate)}</span><span className="block text-xs uppercase tracking-[0.08em] opacity-70">{monthFormatter.format(parsedDate)}</span><span className="mt-1 block text-xs opacity-75">{sessionCount} {sessionCount === 1 ? "class" : "classes"}</span></button>;
          })}
        </div>
        ...
        <div className="mt-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h3 className="text-lg font-semibold">{selectedDate ? fullDayFormatter.format(new Date(selectedDate.starts_at)) : "Schedule"}</h3><p className="text-sm text-black/65" aria-live="polite">{selectedSessions.length} {selectedSessions.length === 1 ? "class" : "classes"} this day · {filteredSessions.length} across 7 days</p></div>
          {scheduleError ? <div role="alert" className="rounded-2xl border border-[#c72c25]/30 bg-[#c72c25]/5 p-5 text-sm text-[#8e211c]">{scheduleError}</div> : selectedSessions.length === 0 ? <div className="rounded-2xl border border-dashed border-black/25 p-7 text-center"><p className="font-semibold">{filtersActive ? "No matches for this day" : "No classes scheduled"}</p><p className="mt-1 text-sm text-black/65">{filtersActive ? "Clear the filters or choose another day." : "Choose another day in the next seven days."}</p>{filtersActive ? <button type="button" onClick={() => { setSelectedClassType("all"); setSelectedInstructor("all"); classSelectRef.current?.focus(); }} className="mt-3 min-h-11 rounded-full border border-black/20 px-5 text-sm font-semibold focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2">Clear filters</button> : null}</div> : (
            <div className="divide-y divide-black/10 border-y border-black/10">
              {selectedSessions.map((session) => {
                const reservation = reservationsBySession.get(session.class_session_id);
                const creditChoiceDisabled = !canBook;
                return <article key={session.class_session_id} className="grid grid-cols-[4.5rem_1fr] gap-x-3 gap-y-3 py-4 lg:grid-cols-[7rem_1fr_minmax(15rem,auto)] lg:items-center"><div><p className="font-mono text-sm font-semibold">{timeFormatter.format(new Date(session.starts_at))}</p><p className="mt-1 flex items-center gap-1 text-xs text-black/65"><Clock3 className="size-3.5" aria-hidden="true" />{Math.round((new Date(session.ends_at).getTime() - new Date(session.starts_at).getTime()) / 60000)} min</p></div><div><div className="flex flex-wrap items-center gap-1.5"><h4 className="text-lg font-semibold tracking-[-0.03em] sm:text-xl">{classNames[session.class_type]}</h4><span className="rounded-full border border-black/15 px-2 py-0.5 text-xs font-semibold uppercase tracking-[0.08em] text-black/65">{session.class_type_label}</span></div><p className="mt-1 text-sm text-black/65">with {session.instructor_name}</p><p className={`mt-1 text-sm font-semibold ${session.is_full ? "text-black/65" : "text-[#8e211c]"}`}>{session.is_full ? `${session.waitlisted_reservations} on the waitlist` : `${session.available_spots} ${session.available_spots === 1 ? "spot" : "spots"} available`}</p></div>{reservation ? <div className="col-span-2 inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-black/15 bg-[#eee6dc] px-5 text-sm font-semibold lg:col-span-1"><Check className="size-4 text-emerald-700" aria-hidden="true" />{reservation.reservation_status === "waitlisted" ? "On waitlist" : "Reserved"}</div> : reservationError ? <p className="col-span-2 max-w-xs text-sm leading-6 text-[#8e211c] lg:col-span-1">Booking is unavailable until your reservations can be verified.</p> : <form action={bookClass} className="col-span-2 grid grid-cols-1 gap-2 min-[420px]:grid-cols-2 lg:col-span-1 lg:flex lg:max-w-sm lg:flex-col lg:justify-self-end"><input type="hidden" name="class_session_id" value={session.class_session_id} /><input type="hidden" name="return_to" value={returnTo} /><DashboardActionButton disabled={creditChoiceDisabled} type="submit" name="use_drop_in" value="false" pendingLabel={session.is_full ? "Joining…" : "Reserving…"} className="h-full min-h-11 rounded-full bg-black px-3 text-xs font-semibold text-white transition hover:bg-[#c72c25] focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:bg-black/25 sm:text-sm">{!summary ? "Eligibility unavailable" : !canBook ? "Membership paused" : session.is_full ? "Waitlist · credit if eligible" : "Reserve · credit if eligible"}</DashboardActionButton><DashboardActionButton disabled={!canBook} type="submit" name="use_drop_in" value="true" pendingLabel={session.is_full ? "Joining…" : "Reserving…"} className="h-full min-h-11 rounded-full border border-black/25 px-3 text-xs font-semibold transition hover:border-black focus-visible:outline-2 focus-visible:outline-[#c72c25] focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-45 sm:text-sm">{session.is_full ? "$35 only if promoted" : "Reserve · simulated $35"}</DashboardActionButton></form>}</article>;
              })}
            </div>
          )}
        </div>
