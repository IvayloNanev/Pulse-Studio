...
export function MemberHome({ account, initialNotificationsOpen = false, memberId, memberSince, notificationError, notifications, summary }: MemberHomeProps) {
  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    <section className="h-full min-w-0">
      <div className="flex h-full min-w-0 flex-col rounded-3xl border border-black/10 bg-[#eee6dc] p-5 sm:p-6 xl:p-7"><p className="font-mono text-xs uppercase tracking-[0.14em] text-black/65">Membership health</p><h2 className="mt-3 min-w-0 break-words text-2xl font-semibold">{summary.plan_name}</h2><div className="mt-3 flex items-center gap-2 text-sm capitalize"><span className={`size-2 rounded-full ${active ? "bg-emerald-700" : "bg-amber-600"}`} aria-hidden="true" />{summary.membership_status}</div><p className="mt-2 text-sm text-black/65">Current cycle ends {cycleFormatter.format(new Date(summary.billing_cycle_end_at))}.</p></div>
    </section>
