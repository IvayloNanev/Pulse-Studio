...
const migration = fs.readFileSync(path.join(root, "supabase/migrations/20260825170000_harden_product_d_shared_security.sql"), "utf8");
...
const checks = [
  ...
  ["uses security definer with fixed search path", (migration.match(/security definer/g) ?? []).length === 2 && (migration.match(/set search_path = public, pg_temp/g) ?? []).length === 2],
  ["validates active staff inside both RPCs", (migration.match(/current_staff_id\(\) is null/g) ?? []).length === 2],
