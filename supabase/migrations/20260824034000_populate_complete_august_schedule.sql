-- Populate every past day in August so the monthly Classes calendar represents
-- the studio's complete schedule, not only the remaining booking window.

begin;

with calendar_days as (
  select day::date as session_date
  from generate_series(date '2026-08-01', date '2026-08-23', interval '1 day') as day
  ...
  select
    'SESSION-AUGUST-' || to_char(session_date, 'YYYYMMDD') || '-' || slot as class_session_id,
    class_type,
    make_timestamptz(
